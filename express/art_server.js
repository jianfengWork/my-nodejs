...
server.get('/query', (req, res, next) => { // 渲染首页
  const content = template(path.join(__dirname, 'art', 'index.art'), {}); // 使用 art 模版
  res.send(content);
});

server.post('/score', (req, res, next) => {
  const result = scoreData[req.body.code]; // no123
  const content = template(path.join(__dirname, 'art', 'result.art'), result);
  res.send(content);
});
...
server.get('/all', (req, res, next) => { // 全部成绩
  ...
  const content = template(path.join(__dirname, 'art', 'all.art'), {
    list: arr
  });
  res.send(content);
});
