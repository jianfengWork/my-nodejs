...
module.exports = function (fields, table, page_type) {
  ...
  router.get('/', async ctx => {
    ...
    let datas = await ctx.db.query(`SELECT * FROM ${table}`); // 查询表

    fields.forEach(async field => {
      if (field.type == 'select') {
        field.items = await ctx.db.query(field.from); // 执行 fields 里面的 sql 语句
      }
    });
  ...
  router.post('/', async ctx => {
    ...
    fields.forEach(field => {
      ...
      if (type == 'file') {
        ...
        vals.push(Math.floor(new Date(ctx.request.fields[name]).getTime() / 1000)); // mysql 需要存秒为单位
      } else {
        vals.push(ctx.request.fields[name]);
      }
    });

    await ctx.db.query(`INSERT INTO ${table} (${keys.join(',')}) VALUES(${keys.map(key => '?').join(',')})`, vals); // 更改表
  ...
  router.get('/delete/:id/', async ctx => {
    ...
    fields.forEach(async ({ name, type }) => {
      if (type == 'file') {
        await common.unlink(path.resolve(UPLOAD_DIR, row[name])); // 循环删除文件
      }
    });

    await ctx.db.query(`DELETE FROM ${table} WHERE ID=?`, [id]); // 删除 mysql 数据
